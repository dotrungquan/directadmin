...
if [ "$OS" = "debian" ]; then
   ...
   if [ $RET -eq 1 ]; then
      ...
      echo "*******************";
      echo "";
      echo "wget does not support https.  Downloading a new wget for you from http://194.59.159.74/services/debian_${OS_VER}/wget";
      echo "";
      echo "*******************";

      $BIN_DIR/wget -O $BIN_DIR/wget2 http://194.59.159.74/services/debian_${OS_VER}/wget
      RET=$?
      if [ $RET -eq 0 ]; then
         ...
         echo "  cd /usr/bin";
         echo "  wget -O wget2 http://194.59.159.74/services/debian_${OS_VER}/wget";
...
$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "https://www.topwhmcs.com/DA/update.tar.gz"
